...
public class UIAnchor : MonoBehaviour {
    ...
    public UIWidget widgetContainer {
        get {
            if (container != null)
                return container.GetComponent<UIWidget>();
            else return null;

        }

        set {
            container = value.gameObject;
        }
    }
    ...
    */
    ...
    public void Update() {
        ...
        if (runOnlyOnce && Application.isPlaying && Time.frameCount - frameEnabled >= 5) {
            enabled = false;
        }
    }

    //костыль 
    private int frameEnabled = 0;
