...
public class UILabel : UIWidget
{
    ...
    UILocalize _localize;
    public UILocalize localize {
        get {
            if (_localize == null) {
                UILocalize rez = GetComponent<UILocalize>();
                if (rez == null) {
                    rez = gameObject.AddComponent<UILocalize>();
                    //rez.key = text;
                }
                _localize = rez;
            }
    ...
    [HideInInspector] [SerializeField] float mDeltaSize = 30;
	...
	public override void OnFill (BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color32> cols)
	{
		...
		{
            float deltaSize = (float)mPrintedSize / (float)mDeltaSize;

			int end = verts.size;
            pos.x = mEffectDistance.x * deltaSize;
            pos.y = mEffectDistance.y * deltaSize;
            Vector2 pos2 = new Vector2();
            pos2.x = mEffectDistance2.x * deltaSize; 
            pos2.y = mEffectDistance2.y * deltaSize;
			...
			if (effectStyle == Effect.Outline)
			{
				...
				ApplyShadow(verts, uvs, cols, offset, end, -pos2.x, pos2.y);
